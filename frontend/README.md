...
The service restricts API paths, injects credentials from the appropriate cookie, validates write origins, preserves ETags and idempotency keys, and deduplicates refresh exchanges. A refresh failure keeps the encrypted cookie unless the backend explicitly rejects the refresh family. Network failures, provider outages, throttling and malformed replies return a retryable error; failed exchanges can be retried immediately with the same deterministic key, including after a gateway restart. An early API authentication rejection gets one refresh and retry with the original request body, organization, testing context and mutation key.

Commit's backend forwards IAM token lifetimes without the original issue time or replay marker. The gateway conservatively subtracts IAM's ten-minute token-response replay window when calculating access expiry, so a replay cannot extend an expired credential. This causes earlier silent renewal, not earlier logout. Browser sessions retain their seven-day maximum, including full test logins following the fifteen-minute sandbox selection step. Recovery of a lost token response remains bounded by IAM's replay window. Keep `SESSION_COOKIE_KEY` stable across restarts and replicas.

`/auth/organizations` forwards the session bearer to Commit's backend without an organization header; the backend uses IAM's live authorization snapshots and returns only organization handles. It requires no IAM app secret, directory token, database, or additional service. Only the existing Commit backend needs its IAM credentials.
