...
```rust
let public = Client::new("https://backend.commit.teamofsilicons.com")?;
...
Version 0.2 adds `claim_project_task`, `delete_project_task`, `project_versions`, `project_version`, `email_settings`, `set_email_settings`, `report`, `contracts`, `testing_context`, and `with_test_app_secret`. See [projects](PROJECTS.md), [testing](TEST_ENVIRONMENTS.md), [contracts](CONTRACTS.md), [telemetry](TELEMETRY.md), and [development](DEVELOPMENT.md).
