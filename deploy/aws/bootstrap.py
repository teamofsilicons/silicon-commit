...
def main(secret_arn, db_host, image, *, root=Path('/opt/commit')):
    os.umask(0o077)
    ...
    try:
        runtime_grants = (root/'postgres_runtime_grants.sql').read_bytes()
        grant_checks = (root/'test_runtime_grants.sql').read_bytes()
        run(psql, input=roles.encode(), stdout=subprocess.DEVNULL)
        running = set(subprocess.check_output([
            'docker', 'ps', '--filter', 'status=running', '--format', '{{.Names}}'
        ], text=True).splitlines())
        previous_running = [name for name in ('commit-api', 'commit-worker') if name in running]
        # Drain both old processes before an incompatible schema can become visible.
        # Allow the API's 30-second graceful shutdown to finish before Docker kills it.
        for name in previous_running:
            run(['docker', 'stop', '--time', '45', name])
        # A failed migration invocation may already have committed earlier migrations.
        # From this point, only the upgraded binaries may be started automatically.
        ...
        run(psql+variables, input=runtime_grants, stdout=subprocess.DEVNULL)
        # Full role contract includes transactional checks of denied writes; all fixtures roll back.
        run(psql+variables, input=grant_checks)
    except BaseException:
        if not migration_started and previous_running:
            # Restore only services that were running before this rollout; never
            # start a previously stopped worker or undo an operator's maintenance.
            subprocess.run(['docker', 'start', *previous_running], check=False)
        raise
    finally:
