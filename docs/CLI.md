...
The CLI records the last crates.io update check in `~/.commit/last-update-check` and checks at most once per hour. Use `--no-update` to disable checks for a run. A newer version is reported on stderr so automation's JSON stdout remains clean.
